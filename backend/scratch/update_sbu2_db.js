    ...
    try {
        ...
        for (const row of excelData) {
            ...
            if (emp) {
                ...
                await EmployeeProfile.findByIdAndUpdate(emp._id, { $set: updateFields });
                updatedCount++;

                // Sync with User if employee has a user profile or email
                if (email) {
        ...
        console.log(`Successfully updated ${updatedCount} Employee Profiles to USGOAN branch with Email & Mobile.`);
        console.log(`Successfully synced ${userSyncedCount} corresponding User accounts.`);
