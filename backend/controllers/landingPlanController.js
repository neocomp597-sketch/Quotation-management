...
// Get all landing plans (seeds automatically if database is empty)
exports.getLandingPlans = async (req, res) => {
    try {
        let plans = await LandingPlan.find().sort({ order: 1 });
        if (plans.length === 0) {
            plans = await LandingPlan.insertMany(DEFAULT_SEED_PLANS);
        }
        res.json({ success: true, data: plans });
    } catch (err) {
        console.error('Error fetching landing plans:', err);
        res.status(500).json({ success: false, message: 'Server error loading plans' });
    }
};

// Create or update a plan tab configuration
exports.saveLandingPlan = async (req, res) => {
    try {
        const { tabKey, tabName, order, tiers } = req.body;
        if (!tabKey || !tabName) {
            return res.status(400).json({ success: false, message: 'Tab key and tab name are required' });
        }

        const cleanTiers = (tiers || []).map(tier => ({
            tierName: tier.tierName || 'TIER',
            subtitle: tier.subtitle || '',
            buttonText: tier.buttonText || 'Free Trial',
            buttonLink: tier.buttonLink || '/register',
            featuresHeader: tier.featuresHeader || 'Essential Features',
            badge: tier.badge || '',
            categories: (tier.categories || []).map(cat => ({
                ...
                items: (cat.items || []).map(item => ({
                    ...
                    isExpandable: Boolean(item.description && item.description.trim()),
                    isIncluded: item.isIncluded !== false
                }))
            }))
        }));

        const plan = await LandingPlan.findOneAndUpdate(
            { tabKey },
            { tabName, order: order || 1, tiers: cleanTiers, updatedAt: new Date() },
            { upsert: true, new: true, setDefaultsOnInsert: true }
        );

        res.json({ success: true, data: plan });
    } catch (err) {
        console.error('Error saving landing plan:', err);
        res.status(500).json({ success: false, message: err.message || 'Server error saving plan' });
    }
};

// Delete a tab configuration
exports.deleteLandingPlan = async (req, res) => {
    try {
        const { tabKey } = req.params;
        await LandingPlan.deleteOne({ tabKey });
        res.json({ success: true, message: 'Tab plan deleted successfully' });
    } catch (err) {
...
// Reset/seed initial default data
exports.seedLandingPlans = async (req, res) => {
    try {
        await LandingPlan.deleteMany({});
        const plans = await LandingPlan.insertMany(DEFAULT_SEED_PLANS);
        if (res && res.json) {
            res.json({ success: true, message: 'Landing plans reset to default seed data', data: plans });
        }
        return plans;
    } catch (err) {
        console.error('Error seeding landing plans:', err);
        if (res && res.status) {
            res.status(500).json({ success: false, message: 'Server error resetting plans' });
        }
    }
};
